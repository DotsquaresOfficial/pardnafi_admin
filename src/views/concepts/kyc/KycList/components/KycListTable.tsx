...
import type { DeleteData, DeleteResponse, GetKycListResponse, Kyc } from '../types'
import type { TableQueries } from '@/@types/common'
import { TbPencil, TbEye, TbLock } from 'react-icons/tb';
import { AiOutlineUnlock } from 'react-icons/ai'
import { updateKycStatus, getAllKyc } from '@/services/KycService'
...
const KycListTable = () => {
    ...
    const handleEdit = (customer: Kyc) => {
        navigate(`/concepts/kyc/kyc-edit/${customer._id}`)
        window.location.reload()
    }

    const handleViewDetails = (customer: Kyc) => {
        navigate(`/concepts/kyc/kyc-details/${customer._id}`)
    }
    ...
    const statusAccountHandler = async (id: string | number, status: boolean): Promise<DeleteResponse> => {
        ...
        try {
            ...
            const data =
                { id, status }
            const response = await updateKycStatus(data, tokenPromise)
            if (response.success) {
    ...
    const columns: ColumnDef<Kyc>[] = useMemo(
        () => [
            {
                header: 'First Name',
                accessorKey: 'name',
    ...
    const handleRowSelect = (checked: boolean, row: Kyc) => {
        setSelectedCustomer(checked, row)
    }

    const handleAllRowSelect = (checked: boolean, rows: Row<Kyc>[]) => {
        if (checked) {
    ...
    return (
        <DataTable
            ...
            columns={columns}
            data={kycList}
            noData={!isLoading && kycList.length === 0}
